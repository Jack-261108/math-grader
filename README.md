...
> 💡 支持手机微信 / 移动浏览器 / 电脑大屏直接访问；支持 **BYOK (Bring Your Own Key)** 本地自带密钥模式，配置仅存本地浏览器，安全无忧。
...
4. **持久化错题知识库 & 艾宾浩斯抗遗忘复习流**：基于 SQLite 结构化持久存储，批改错题自动无感增量去重入库；二级核心考点智能聚类，严格遵循**艾宾浩斯遗忘曲线（1天/3天/7天/15天）**调度复习，支持一键组装 **15 题专项提分重练卷**；
5. **错题名师秒杀点拨与速算公式知识库**：一键调用大模型深度剖析错因与公考破局技巧，内置行测核心速算公式与模块口诀库，支持一键复制与引用追问；
6. **纯前端 A4 错题本与空白重做卷排版引擎**：纯前端矢量排版，支持一键打印或导出高质量 A4 纯净空白重做卷与错题复盘解析版；
...
### 🧠 持久化错题知识库 & 艾宾浩斯抗遗忘复习流 (`wrong_book_db.py` / `WrongBookView.vue`)
- **结构化 SQLite 本地持久化**：
  - 无论速算练习册还是行测答题卡，批改完成后错题**全自动无感增量沉淀入库**至本地 SQLite 数据库（`data/wrong_book.db`）；
  - 严格规范题目算式指纹哈希（Expression Fingerprint），杜绝重复入库与指纹哈希碰撞。
- **二级核心考点智能聚类与错因打标**：
  - 自动映射二级核心考点（如行测：两期比重、增长率比较、削弱论证、前提假设；速算：截位直除、退位借位、进位加法等）；
  - 支持错因标签打标（粗心失误 `careless`、概念混淆 `confused`、蒙题猜测 `guessing`、计算偏差 `calculation`、完全未学 `unfamiliar`）与个人复盘笔记编辑。
- **艾宾浩斯抗遗忘记忆曲线调度**：
  - 严格依据遗忘曲线（第 1 天、第 3 天、第 7 天、第 15 天）精准调度「今日待复习」错题任务；
  - 连续 2 次答对自动移入「已攻克」知识库，答错自动回退至上一阶段循环加固；
  - 页面顶部导航栏与标签页实时提供待复习错题红点未核销计数提醒。
- **弱项重练组卷中心**：
  - 支持按薄弱二级考点或今日待复习错题**一键组装 15 题专项提分重练卷**；
  - 既支持在网页端直接在线心算刷题即判，也支持一键导出排版规整的 A4 纸质空白重做卷。
...
- 状态按领域分层解耦（`math`, `omr`, `wrongbook`, `modal`, `history`, `config`, `tutor`）；
...
├── wrong_book_db.py          # 持久化错题知识库核心（SQLite 存储、艾宾浩斯抗遗忘调度、15题提分卷）
...
├── server.py                 # FastAPI 移动端与全功能 Web 服务（速算、答题卡、错题库、名师接口）
├── deploy.sh                 # 一键拉取最新代码、字体检测、依赖同步、前端编译与服务平滑重启脚本
├── data/
│   └── wrong_book.db         # 持久化错题知识库与艾宾浩斯复习流水 SQLite 数据库
├── frontend/                 # 现代化 Vue 3 + Pinia + Vue Router + Vite 前端工程
│   ├── src/
│   │   ├── api/              # API 异步请求层（速算、答题卡、错题本、名师、历史记录等）
│   │   ├── components/       # 业务组件库（速算、答题卡、伴考计时器、四象限诊断、通用模态框等）
│   │   ├── constants/        # 公考考场时序、速算核心公式库口诀、常规模板常量
│   │   ├── stores/           # Pinia 响应式状态管理（速算、答题卡、错题本、弹窗、全局配置等）
│   │   ├── utils/            # 工具库（常亮防熄屏、全屏台钟、图片压缩、格式化等）
│   │   └── views/            # 核心业务视图（MathView 速算、OmrView 答题卡、WrongBookView 错题库）
│   ├── package.json          # 前端依赖配置
...
### 场景五：艾宾浩斯抗遗忘错题复习与考前 A4 重做
1. 切换至 **【📚 错题知识库】**（红色角标提示今日到期题数）；
2. 系统已按艾宾浩斯遗忘曲线自动筛选出今日待复习错题，可直接在线重做，连续答对 2 次即攻克；
3. 支持点击 **【组装15题专项提分卷】** 针对性强化；
4. 点击 **【🖨️ A4 打印】**，选择「纯净空白重刷卷」，一键导出不含答案与笔记的独立纸质卷考前实战模拟！
