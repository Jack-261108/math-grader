...
def mat_to_data_url(mat: np.ndarray, quality: int = 85) -> str:
    """将 OpenCV 图像在内存中编码为 base64 data url，零磁盘写入"""
...
async def api_grade(
    ...
    try:
        ...
        return {
            ...
            "orig_url": result_data.get("orig_url", ""),
...
async def api_grade_omr(
    ...
    try:
        ...
        # 5. 渲染批注答题卡全景图（纯内存 Base64 编码，零文件落盘）
        if img is not None:
        ...
        card_data_url = mat_to_data_url(card_bgr, quality=85)
        ...
        logger.info(f"[OMR-Grade] 判分与批注完成 (纯内存零文件): task_id={task_id}, 试卷={exam_title_str}, 得分={earned_score}/{full_score}分 ({score_ratio}%), 等级={grade_badge}, 对/错/空={tot_correct}/{tot_wrong}/{tot_unans}, 耗时={elapsed_ms}ms")
